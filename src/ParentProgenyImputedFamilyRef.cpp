...
//////////////////// ParentProgenyImputedFamilyRef ////////////////////

bool ParentProgenyImputedFamilyRef::is_small(const vector<vector<int>>& ref_haps,
...
size_t ParentProgenyImputedFamilyRef::compute_upper_num_progenies(
...
const KnownFamily *ParentProgenyImputedFamilyRef::reduce_progenies(
...
VCFImputable *ParentProgenyImputedFamilyRef::create_family_vcf(
...
VCFGeno *ParentProgenyImputedFamilyRef::impute(const VCFSmall *orig_vcf,
	...
	for(auto p = families.begin(); p != families.end(); ++p) {
		...
		auto	*vcf1 = create_family_vcf(family, records,
											families.size(),
											ref_haps, should_impute_mat,
											orig_vcf, op);
		vcfs.push_back(vcf1);
