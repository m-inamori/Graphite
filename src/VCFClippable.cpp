#include "../include/VCFClippable.h"

using namespace std;

vector<int> VCFClippable::get_int_gts(size_t sample_index) const {
	vector<int>	int_gts;
...
Haplotype VCFClippable::clip_haplotype(size_t sample_index, int side) const {
	const auto	hap = this->clip_raw_haplotype(sample_index, side);
	return Haplotype(hap, sample_index, side);
}

HaplotypePair VCFClippable::impute_cM_each_sample(HaplotypePair prev_hap,
													size_t sample_index,
													bool exec) {
...
void VCFClippable::set_haplotype(HaplotypePair hap, size_t sample_index) {
