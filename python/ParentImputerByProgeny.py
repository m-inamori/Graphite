...
class ParentImputerByProgeny(VCFHMM[VCFFamilyRecord]):
	...
	def emission_probability(self, i: int, h: int) -> float:
		...
		parent_gt = self.records[i].unphased(index)
	...
	def update_genotypes(self, hs: list[int]) -> None:
		...
		is_swapped = self.should_impute_mat ^ (hs[0] & 1) == 0
		index = self.should_imputed_index()
		for i in range(M):
			record = self.records[i]
			parent_gt = self.compute_parent_gt(i, hs[i])
			record.geno[index] = parent_gt | 4
			if is_swapped:
				gt = record.geno[index]
				record.geno[index] = Genotype.inverse(gt)
