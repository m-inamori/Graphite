...
class ParentRefImputer(VCFHMMRef[VCFFamilyRecord]):
	...
	def calc_Epc(self, w: float) -> list[list[list[float]]]:
		...
		E1 = [ [ [0.0]*5 for _ in range(4) ] for _ in range(4) ]
		for mat, pat in product(range(4), range(4)):
			orig_prob = [0.0] * 5
			for a1, a2 in product((mat&1, mat>>1), (pat&1, pat>>1)):
				gt = a1 | (a2 << 1)
				if gt == 1:
