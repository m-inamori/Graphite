...
class VCFSelfFillableRecord(VCFRecord):
	...
	def gt_from_parent(self, mat_from: int, pat_from: int) -> str:
		...
		gt_from_pat = '.' if pat_from == 0 else self.v[9][pat_from*2-2]
	...
	@staticmethod
	def convert(record: VCFImpSelfRecord) -> VCFSelfFillableRecord:
		type = record.get_fill_type()
		probs = record.parse_PL()
		return VCFSelfFillableRecord(record.v, record.samples, record.index,
													type, record.pair, probs)
	
	@staticmethod
	def merge(records: list[VCFSelfFillableRecord],
	...
	@staticmethod
	def decide_duplicated_Genotype(records: list[VCFSelfFillableRecord],
	...
	@staticmethod
	def integrate_each_sample(records: list[VCFSelfFillableRecord],
										ps: list[tuple[int, int]]) -> None:
		GT = VCFSelfFillableRecord.decide_duplicated_Genotype(records, ps)
	...
	@staticmethod
	def integrate(records: list[VCFSelfFillableRecord], samples: list[str],
						pos_samples: list[list[tuple[int, int]]]) -> VCFRecord:
		# 0|0 x 1|1ならひっくり返せる
		for ps in pos_samples:
			GTs = [ records[i].v[j+9][:3] for i, j in ps ]
			if not is_all_same(GTs):
				VCFSelfFillableRecord.integrate_each_sample(records, ps)
