...
class VCFNoParentKnown(VCFBase, VCFSmallBase):
	def __init__(self, header: list[list[str]], records: list[VCFFamilyRecord],
		...
		self.parent_imputer1 = ParentImputer(records, is_mat_known,
														ref_haps, map_, 0.01)
		self.parent_imputer2 = ParentImputer(records, not is_mat_known,
														ref_haps, map_, 0.01)
