...
class VCFHeteroImpHomo(VCFHeteroHomoOnePhased):
	...
	def impute(self):
		...
		def next_states_large(state: int,
							record: VCFRecord) -> Iterator[tuple[int, int]]:
			n = self.num_progenies()
			_, prev_hs, prev_num_crossovers = decode(state, n)
			...
			for order in (0, 1):
				...
				s = order | (prev_num_crossovers << (n+1))
				mis = 0
		...
		def next_states_small(state: int,
							record: VCFRecord) -> Iterator[tuple[int, int]]:
			n = self.num_progenies()
			_, v, prev_num_crossovers = decode(state, n)
			# order 0 -> '0|1', 1 -> '1|0'
			for s in range(2**(n+1)):
				order = s & 1
				w = [ (s >> i) & 1 for i in range(1, n+1) ]
				mis = 0
				num_crossovers = prev_num_crossovers
				gt_imputed = record.get_int_gt(self.imputed_index())
				for i in range(2, n+2):
					gt = record.get_int_gt(i)
					prev_h = v[i-2]
					...
					if prev_h != h:
						num_crossovers += 1
					if gt != -1 and gt != gt_imputed // 2 + (order ^ h):
						mis += 1
				if num_crossovers <= n * 2:
					s1 = s | (num_crossovers << (n+1))
					yield (s1, mis)
		...
		def next_states(state: int,
				...
				n = self.num_progenies()
				if n < 5:
					for pair in next_states_small(state, record):
						yield pair
				else:
					for pair in next_states_large(state, record):
						yield pair
