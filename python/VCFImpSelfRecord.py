...
class VCFImpSelfRecord(VCFRecord, metaclass=ABCMeta):
	...
	@staticmethod
	def which_is_fixed(v: list[tuple[VCFImpSelfRecord, int]]
						) -> tuple[int, list[tuple[VCFImpSelfRecord, int]]]:
	...
	@staticmethod
	def modify_00x11(records: list[VCFImpSelfRecord]) -> None:
		# { sample: [(VCFImpSelfRecord, mat or pat)] }
		...
		for v in dic.values():
			...
			right_gt, wrongs = VCFImpSelfRecord.which_is_fixed(v)
