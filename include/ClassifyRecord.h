...
public:
	...
	typedef std::pair<ParentComb, double>	GTComb;
	
private:
	// Singleton
	static ClassifyRecord	*instance;
	
	std::map<std::size_t, const TypeDeterminer *>	memo;
...
public:
	...
	const TypeDeterminer *get_TypeDeterminer(std::size_t n);
...
private:
	std::array<int, 4> count_int_gts(const std::vector<int>& gts) const;
	std::array<int, 4> count_int_gts(std::vector<int>::const_iterator first,
